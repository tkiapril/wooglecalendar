...
This file is derived from cliche/orm.py file from Cliche_ project.
...
def get_alembic_config(engine):
    ...
    a string of database url either.  So::
        ...
        engine = create_engine('postgresql://localhost/wooglecalendar')
        alembic_cfg = get_alembic_config(engine)

    is equivalent to::

        db_url = 'postgresql://localhost/wooglecalendar'
    ...
    cfg = Config()
    cfg.set_main_option('script_location', 'wooglecalendar:migrations')
...
def import_all_modules():
    """Import all submodules of :mod:`wooglecalendar` to ensure every ORM
    ...
    for _, mod, __ in pkgutil.walk_packages(current_dir):
        if mod.startswith('wooglecalendar.'):
            __import__(mod)